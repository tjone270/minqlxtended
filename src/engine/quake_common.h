...
#include <stddef.h> // offsetof, for the layout assertions at the end of the struct definitions
#include <stdint.h>
#include <string.h> // strlen/strncmp, for cmd_word_is below
...
// 702-703 unidentified. The [G_InitGame] entries are the ones qagamex64.so build 1069
// writes directly; the rest are positioned to fit between them.
#define CS_CUSTOM_SETTINGS 704            // [G_InitGame] 0x2c0, g_customSettings
#define CS_ROTATIONMAPS 705
#define CS_ROTATIONVOTES 706
#define CS_DISABLE_VOTE_UI 707
#define CS_ALLREADY_TIME 708
#define CS_INFECTED_SURVIVOR_MINSPEED 709 // [G_InitGame] 0x2c5, g_rrInfectedSurvivorMinSpeed
#define CS_RACE_POINTS 710
#define CS_DISABLE_LOADOUT 711    // [G_InitGame] 0x2c7, g_disableLoadout
// 712 and 714 come from observed traffic, not the listing: the [G_InitGame] note for 712 reads
// trap_GetServerSteamID() and disagrees. Every other index in this block checks out literally.
#define CS_MATCH_GUID 712         // also sent in the ZMQ stats; the demo/match join key
#define CS_STARTING_WEAPONS 713   // [G_InitGame] 0x2c9, g_startingWeapons; on the client ESC menu
#define CS_STEAM_ID 714           // the server's own SteamID64. It ROTATES - never an identity
...
/*
 * Cvar_Get at qzeroded 0x426bc0 does `slotOffset = cvar_numIndexes * 0x68` into cvar_indexes, so
 * the stride is the size. Cvar.flags, .modified and .modification_count are written straight
 * through from Python, and a longer header puts one of those into the next cvar.
 */
_Static_assert(sizeof(cvar_t) == 0x68, "cvar_t must match qzeroded; Cvar_Get strides by 0x68 at 0x426bc0");
...
typedef struct playerState_s {
    ...
    int persistant[MAX_PERSISTANT]; // +256
    int powerups[MAX_POWERUPS];     // +320
...
/*
 * The code indexes on these strides: svs->clients[i], level->clients[i], cl - svs->clients.
 * A wrong size shifts every element past index 0, and scoreboard.c writes through those.
 */
_Static_assert(sizeof(client_t) == 158328, "client_t must match qzeroded.x64 build 1069");
_Static_assert(sizeof(netchan_t) == 65596, "netchan_t must match qzeroded.x64 build 1069");

/*
 * Offsets as well as sizes, for every client_t field the codebase dereferences. Sizes alone
 * do not catch a field being moved within the struct, which is how level_locals_t went wrong.
 * Each figure below is a literal displacement out of qzeroded.x64 build 1069:
 *
 *   SV_SendClientMessages   ADD RBX, 0x26a78          sizeof(client_t)      158328
 *   SV_DropClient           CMP [RDI], 1              state                      0
 *                           MOV RAX, [RDI + 0x10840]  gentity              0x10840
 *   SV_AddServerCommand     MOV EAX, [RDI + 0x10404]  reliableSequence     0x10404
 *                           SUB EDX, [RDI + 0x10408]  reliableAcknowledge  0x10408
 *   SV_SendClientGameState  MOV [RBP + 0x10414], EAX  gamestateMessageNum  0x10414
 *                           MOV EAX, [RBP + 0x16a0c]  netchan.outgoingSequence
 *                           LEA RBX, [RDI + 0x10848]  name                 0x10848
 *   SV_SendMessageToClient  MOV [R12 + 0x11b58], EDX  frames[0].messageSize
 *                           MOV EDX, [RDI + 0x14]     msg_t.cursize             20
 */
_Static_assert(offsetof(client_t, state) == 0, "client_t.state moved");
_Static_assert(offsetof(client_t, reliableSequence) == 0x10404, "client_t.reliableSequence moved; the reliable guard tracks it");
_Static_assert(offsetof(client_t, reliableAcknowledge) == 0x10408, "client_t.reliableAcknowledge moved; the reliable guard tracks it");
_Static_assert(offsetof(client_t, gamestateMessageNum) == 0x10414, "client_t.gamestateMessageNum moved");
_Static_assert(offsetof(client_t, gentity) == 0x10840, "client_t.gentity moved");
_Static_assert(offsetof(client_t, name) == 0x10848, "client_t.name moved");
_Static_assert(offsetof(client_t, netchan) + offsetof(netchan_t, outgoingSequence) == 0x16a0c,
               "client_t.netchan.outgoingSequence moved; the demo writer keys off it");
_Static_assert(offsetof(client_t, frames) + offsetof(clientSnapshot_t, messageSize) == 0x11b58,
               "client_t.frames layout moved");
_Static_assert(sizeof(clientSnapshot_t) == 648, "clientSnapshot_t must match qzeroded.x64 build 1069");
_Static_assert(sizeof(netadr_t) == 20, "netadr_t must match qzeroded.x64 build 1069");

// msg_t.cursize and .data are what the demo tap copies out of every outgoing message.
_Static_assert(sizeof(msg_t) == 32, "msg_t must match qzeroded.x64 build 1069");
_Static_assert(offsetof(msg_t, data) == 8, "msg_t.data moved; the demo tap reads it");
_Static_assert(offsetof(msg_t, cursize) == 20, "msg_t.cursize moved; the demo tap reads it");

// SV_DropClient tests SVF_BOT as "TEST byte ptr [RAX + 0x1e0], 8" on client->gentity.
_Static_assert(offsetof(sharedEntity_t, r) + offsetof(entityShared_t, svFlags) == 0x1e0,
               "sharedEntity_t.r.svFlags moved");
...
/*
 * svs is a global at 0x1472280 in qzeroded.x64 build 1069. SV_SendClientMessages reads
 * svs.time as an absolute at 0x1472284 and svs.clients at 0x1472290, and SV_DropClient
 * walks the challenges array from 0x14722a8 with a 0x2b8 stride.
 */
_Static_assert(sizeof(challenge_t) == 696, "challenge_t must match qzeroded.x64 build 1069");
_Static_assert(offsetof(serverStatic_t, time) == 4, "serverStatic_t.time moved");
_Static_assert(offsetof(serverStatic_t, clients) == 16, "serverStatic_t.clients moved; every client lookup goes through it");
_Static_assert(offsetof(serverStatic_t, challenges) == 40, "serverStatic_t.challenges moved");
...
/*
 * sv is a global at 0x15202e0 in qzeroded.x64 build 1069, immediately after sv_pure, derived from
 * SV_GetConfigstring's read of sv.configstrings (see OFFSET_PP_SV_CONFIGSTRINGS). Get that
 * member's offset wrong and the whole struct is placed wrong. The layout is stock Q3 and Ghidra
 * confirms it in 1069: SV_SetConfigstring compares sv.state at 0x15202e0 against SS_GAME and
 * reads sv.restarting at 0x15202e4.
 */
_Static_assert(offsetof(server_t, state) == 0, "server_t.state moved");
_Static_assert(offsetof(server_t, restarting) == 4, "server_t.restarting moved");
_Static_assert(offsetof(server_t, configstrings) == 2088,
               "server_t.configstrings moved; sv itself is derived by subtracting this");
_Static_assert(offsetof(server_t, restartTime) == 354380, "server_t.restartTime moved");
_Static_assert(sizeof(server_t) == 354384, "server_t must match qzeroded.x64 build 1069");
...
/*
 * bg_itemlist is an array, so its stride is the size: BG_FindItemForWeapon at qagame 0x153360
 * returns `&bg_itemlist + index * 0x78`. Nothing writes through this one - see ITEM_FIELDS in
 * engine_fields.h - but Item(n) still indexes by it, so a wrong size reads the wrong item.
 */
_Static_assert(sizeof(gitem_t) == 0x78, "gitem_t must match qagamex64.so; BG_FindItemForWeapon strides by 0x78 at 0x153360");
...
/*
 * Verified against qagamex64.so build 1069. The stride comes from the g_entities walks in
 * CA_RoundStateTransition (0x420). die comes from ClientSpawn's "MOV [R14+0x388], RAX", the
 * pointer the death events hang off. Touch_Item bumps pickupCount once per successful pickup.
 */
_Static_assert(sizeof(gentity_t) == 1056, "gentity_t must match qagamex64.so build 1069");
_Static_assert(offsetof(gentity_t, die) == 0x388, "gentity_t.die moved; the death events read it");
_Static_assert(offsetof(gentity_t, pickupCount) == 1048, "gentity_t.pickupCount moved; item_pickup diffs it");
/*
 * Touch_Item, in one stretch: "MOV RCX, [RBX + 0x240]" is other->client, "[RBX + 0x260]" is
 * other->flags, "MOV RAX, [RBP + 0x410]" is ent->item, "CMP [RAX + 0x64], 5" is
 * item->giType == IT_POWERUP, and "MOV RDX, [RAX]" is the classname handed to
 * G_LogPrintf("Item: %i %s\n"), the same field item_pickup reports.
 */
_Static_assert(offsetof(gentity_t, client) == 0x240, "gentity_t.client moved");
_Static_assert(offsetof(gentity_t, flags) == 0x260, "gentity_t.flags moved");
_Static_assert(offsetof(gentity_t, item) == 0x410, "gentity_t.item moved; item_pickup reads it");
_Static_assert(offsetof(gitem_t, classname) == 0, "gitem_t.classname moved; item_pickup reports it");
_Static_assert(offsetof(gitem_t, giType) == 0x64, "gitem_t.giType moved");
_Static_assert(offsetof(entityState_t, number) == 0, "entityState_t.number moved");
...
_Static_assert(sizeof(gclient_t) == 3064, "gclient_t must match qagamex64.so build 1069");
_Static_assert(sizeof(expandedStatObj_t) == 812, "expandedStatObj_t must match qagamex64.so build 1069");

/*
 * Sub-offsets within gclient_t and playerState_t, from literal displacements in
 * qagamex64.so build 1069. CA_RoundStateTransition alone pins most of them:
 *
 *   [client + 0x250] != 2          pers.connected != CON_CONNECTED       pers   592
 *   [client + 0x348] == 3          sess.sessionTeam == TEAM_SPECTATOR    sess   840
 *   [client + 0xc]  |= 4           ps.pm_flags                                   12
 *   [client + 0x140] = g_spawnArmor + ...   ps.powerups[PW_SPAWNARMOR]          320
 *   [client + 0x88]                ps.clientNum, passed to trap_GetSteamStat    136
 *   ([client+0xbe4] * 100) / [client+0xbe0]   the CA accuracy medal: hits/shots
 *   [client + 0xbe8] == 0          the CA perfect medal: no damage taken
 *
 * The two strides are Touch_Item's denial loop (0xbf8 per client) and the g_entities walks
 * in CA_RoundStateTransition (0x420 per entity).
 */
_Static_assert(offsetof(gclient_t, ps) == 0, "gclient_t.ps moved");
_Static_assert(offsetof(gclient_t, pers) == 592, "gclient_t.pers moved");
_Static_assert(offsetof(gclient_t, sess) == 840, "gclient_t.sess moved");
_Static_assert(offsetof(gclient_t, expandedStats) == 1400, "gclient_t.expandedStats moved; Player.expanded_stats reads it");
_Static_assert(offsetof(gclient_t, round_shots) == 3040, "gclient_t.round_shots moved");
_Static_assert(offsetof(gclient_t, round_hits) == 3044, "gclient_t.round_hits moved");
_Static_assert(offsetof(gclient_t, round_damage) == 3048, "gclient_t.round_damage moved");
_Static_assert(offsetof(clientPersistant_t, connected) == 0, "clientPersistant_t.connected moved; the team poll tests it");
_Static_assert(offsetof(clientSession_t, sessionTeam) == 0, "clientSession_t.sessionTeam moved; team_switch edge-detects it");

_Static_assert(sizeof(playerState_t) == 592, "playerState_t must match qagamex64.so build 1069");
_Static_assert(offsetof(playerState_t, pm_flags) == 12, "playerState_t.pm_flags moved");
_Static_assert(offsetof(playerState_t, clientNum) == 136, "playerState_t.clientNum moved");
_Static_assert(offsetof(playerState_t, stats) == 192, "playerState_t.stats moved");
_Static_assert(offsetof(playerState_t, persistant) == 256, "playerState_t.persistant moved");
_Static_assert(offsetof(playerState_t, powerups) == 320, "playerState_t.powerups moved");
_Static_assert(offsetof(playerState_t, ammo) == 384, "playerState_t.ammo moved");
...
typedef struct __attribute__((aligned(8))) {
    ...
    int timeOvertime;
    /*
     * Easy to miss, and roundState and everything after it shifts if you do.
     * G_SpawnEntitiesFromString fills both with "<delay> + random() * <random>" seconds, and
     * FinishSpawningItem defers an item's first spawn by whichever applies: keys here, powerups
     * below. The two writes are level+0x1de0 and level+0x1de4 on qagamex64.so build 1069.
     * bodyQue is 8-aligned, so dropping one int here moves the back half by eight bytes.
     */
    int timeInitialKeySpawn;     // +7648, from g_spawnDelay_key / g_spawnDelayRandom_key
    int timeInitialPowerupSpawn; // +7652, from g_spawnDelay_powerup / g_spawnDelayRandom_powerup
    int bodyQueIndex;            // +7656
    gentity_t *bodyQue[BODY_QUEUE_SIZE]; // +7664, after 4 bytes of alignment padding
    int portalSequence;
    ...
    team_t attackingTeam;    // +10628
    roundState_t roundState; // +10632
    ...
    qboolean matchForfeited; // +19916. ForfeitMatch writes 1 here (0x91a34c).
    int allReadyTime;
...
} level_locals_t; // Verified from qagamex64.so build 1069.

/*
 * Offsets as well as sizes. A wrong offset returns a neighbouring field instead of failing, so
 * it has to be asserted: this struct carried a "verified" comment while roundState and
 * everything after it sat 8 bytes early.
 *
 * level is at 0x915580 in build 1069 (the GOT slot at 0x3ce9d8), so each figure below is the
 * absolute address in the listing minus that base:
 *
 *   BeginIntermission      reads  0x917320 -> intermissionTime  7584
 *   AD_RoundStateTransition writes 0x917f04 -> attackingTeam    10628
 *   CA_GetRoundState       reads  0x917f08 -> roundState        10632  (its eCurrent)
 *   CA/RR/Freeze transitions use  0x917f1c -> roundState.round  10652
 *   ForfeitMatch           writes 0x91a34c -> matchForfeited    19916
 */
_Static_assert(offsetof(level_locals_t, time) == 36, "level_locals_t.time moved");
_Static_assert(offsetof(level_locals_t, teamScores) == 48, "level_locals_t.teamScores moved; round_end diffs it");
_Static_assert(offsetof(level_locals_t, intermissionQueued) == 7580, "level_locals_t.intermissionQueued moved; game_end watches it");
_Static_assert(offsetof(level_locals_t, intermissionTime) == 7584, "level_locals_t.intermissionTime moved");
/*
 * These bracket the easiest spot to be short an int. bodyQue is asserted as well because
 * it is 8-aligned, so a lost or gained field ahead of it can hide in padding.
 */
_Static_assert(offsetof(level_locals_t, timeInitialKeySpawn) == 7648, "level_locals_t.timeInitialKeySpawn moved");
_Static_assert(offsetof(level_locals_t, timeInitialPowerupSpawn) == 7652, "level_locals_t.timeInitialPowerupSpawn moved");
_Static_assert(offsetof(level_locals_t, bodyQueIndex) == 7656, "level_locals_t.bodyQueIndex moved");
_Static_assert(offsetof(level_locals_t, bodyQue) == 7664, "level_locals_t.bodyQue moved");
_Static_assert(offsetof(level_locals_t, mapIsTrainingMap) == 7736, "level_locals_t.mapIsTrainingMap moved; Game reads it");
_Static_assert(offsetof(level_locals_t, numRedArmorPickups) == 10264, "level_locals_t.numRedArmorPickups moved");
_Static_assert(offsetof(level_locals_t, attackingTeam) == 10628, "level_locals_t.attackingTeam moved");
_Static_assert(offsetof(level_locals_t, roundState) == 10632, "level_locals_t.roundState moved; round_end watches it");
_Static_assert(offsetof(level_locals_t, matchForfeited) == 19916, "level_locals_t.matchForfeited moved; game_end reports it");
/*
 * The tail past the 9 KiB of intermission map strings. matchForfeited is the last field with a
 * listing address of its own and six ints follow it, so these bracket the other easy spot to be
 * short one. The end bound comes from the engine: G_InitGame does memset(&level, 0, 0x4de8) at
 * 0x1806c5, and a field declared past that is a Python-writable accessor pointing into whatever
 * follows `level`. 19944 is equally consistent with the last member ending at 19940 plus four
 * bytes of tail padding, so lastLeadChangeElapsedTime (19940, no xrefs) is unverified either way.
 */
_Static_assert(offsetof(level_locals_t, intermissionMapVotes) == 19904, "level_locals_t.intermissionMapVotes moved");
_Static_assert(sizeof(level_locals_t) == 19944, "level_locals_t must match qagamex64.so build 1069; G_InitGame memsets 0x4de8 at 0x1806c5");

/*
 * CA_GetRoundState is the whole struct in miniature: it returns eCurrent, copies eNext
 * into it once level.time passes tNext, and stamps startTime with level.time on the way
 * through.
 */
_Static_assert(sizeof(roundState_t) == 36, "roundState_t must match qagamex64.so build 1069");
_Static_assert(offsetof(roundState_t, eCurrent) == 0, "roundState_t.eCurrent moved; round_end edge-detects it");
_Static_assert(offsetof(roundState_t, round) == 20, "roundState_t.round moved");
...
extern server_t *sv; // NULL if the displacement it is derived from stopped making sense
...
/*
 * Head of the engine's cvar list, for enumeration. A pointer to the engine's own head
 * pointer, so it tracks cvars registered after we resolved it. NULL if the offset did
 * not look right; see OFFSET_RELP_CVAR_VARS.
 */
...
typedef void(__cdecl *SV_UpdateUserinfo_f_ptr)(client_t *cl);
...
typedef void(__cdecl *SV_UnlinkEntity_ptr)(sharedEntity_t *gEnt);
...
// mode is SAY_ALL / SAY_TEAM / SAY_TELL; target is only used by SAY_TELL.
typedef void(__cdecl *G_Say_ptr)(gentity_t *ent, gentity_t *target, int mode, const char *chatText);
typedef void(__cdecl *SetTeam_ptr)(gentity_t *ent, char *s);
typedef void(__cdecl *FireWeapon_ptr)(gentity_t *ent);
...
// No __cdecl, so this stays assignment-compatible with gentity_t.die, which is declared
// without it.
typedef void (*player_die_ptr)(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod);
...
// leave RAX as garbage, so the caller has to find the new entity another way.
typedef void(__cdecl *G_SpawnGEntityFromSpawnVars_ptr)(void);
typedef void(__cdecl *SelectScoreboardMessage_ptr)(gentity_t *ent);
typedef qboolean(__cdecl *MP_AllowJoin_ptr)(gentity_t *ent, int team);
typedef void(__cdecl *MP_LockOrUnlockTeam_ptr)(int team, int locked);
typedef void(__cdecl *MP_PauseThink_ptr)(void);
typedef void(__cdecl *MP_StopDemo_ptr)(void);
...
extern SV_Shutdown_ptr SV_Shutdown; // Used to get the svs pointer, then hooked to finalise demos.
extern SV_Map_f_ptr SV_Map_f;       // Used to get Cmd_Argc
extern SV_UpdateUserinfo_f_ptr SV_UpdateUserinfo_f;
...
extern SV_LinkEntity_ptr SV_LinkEntity;     // NULL if the pattern did not match
extern SV_UnlinkEntity_ptr SV_UnlinkEntity; // NULL if the pattern did not match
...
/*
 * Where Cmd_CallVote_f was found, captured before it is hooked. Hook() overwrites the
 * pointer above with the trampoline, and the callvote-clientkick patch is an offset into
 * the real function body, so it has to be based on this instead.
 */
extern pint Cmd_CallVote_f_addr;
extern G_Say_ptr G_Say;
extern SetTeam_ptr SetTeam;
extern FireWeapon_ptr FireWeapon;

// G_Say's mode argument. Verified against the CMP EBP comparisons in qagame 1069.
#define SAY_ALL 0
#define SAY_TEAM 1
#define SAY_TELL 2
extern G_Damage_ptr G_Damage;
extern player_die_ptr player_die;
...
extern SelectScoreboardMessage_ptr SelectScoreboardMessage; // NULL if the pattern did not match
extern MP_LockOrUnlockTeam_ptr MP_LockOrUnlockTeam;         // the only writer of mp_teamLocked; NULL if unresolved
// Anchors only. Each is pattern-matched for the address in its opening instruction and never
// called; see the match-play globals below.
extern MP_AllowJoin_ptr MP_AllowJoin;
extern MP_PauseThink_ptr MP_PauseThink;
extern MP_StopDemo_ptr MP_StopDemo;

/*
 * The game module's match-play globals, at qagame+0x6e3480..0x6e34ac in build 1069. These are
 * *not* a struct: they are separate file-scope globals the compiler laid out consecutively, so
 * MP_AllowJoin reaches the locks as base+0x1C and Cmd_TimeOut the timeouts as base+0xC. Each gets
 * its own pointer and its own provenance:
 *
 *   mp_unpauseTime      MP_PauseThink's opening instruction, direct. Also the base the other
 *                       five are measured from, so SearchVmFunctions cross-checks they agree.
 *   mp_pauseCaller      base+0x04. MP_Pause writes it directly, but see patterns.h for why
 *                       that is not usable as an anchor.
 *   mp_pausedByServer   base+0x08, as above.
 *   mp_timeoutsUsed     base+0x0C, the displacement Cmd_TimeOut indexes it by.
 *   mp_teamLocked       base+0x1C. The soundest of the six: that displacement is a must-match
 *                       byte in both PTRN_MP_ALLOWJOIN and PTRN_MP_LOCKORUNLOCKTEAM, so moving
 *                       the locks breaks the pattern match rather than shifting a read.
 *   mp_autoActionState  MP_StopDemo's opening instruction, direct. Cross-checked against
 *                       base+0x2C, which pins the far end of the span.
 *
 * All six are NULL together, so one test covers them. G_InitGame clears none of it: MP_Init only
 * re-publishes configstrings 669-672 from whatever is already there, so a lock and a running
 * timeout both outlive a map change.
 */
extern int *mp_unpauseTime;         // level.time the timeout ends; 0 for an indefinite pause
extern int *mp_pauseCaller;         // client num that called it, 0 when the server did
extern qboolean *mp_pausedByServer;
extern int *mp_timeoutsUsed;        // TEAM_NUM_TEAMS wide; only RED and BLUE are ever written
extern int *mp_teamLocked;          // TEAM_NUM_TEAMS wide; only RED and BLUE gate joining
extern int *mp_autoActionState;     // demo/screenshot auto-action state
...
// Print through the console-print hook so it dispatches "console_print" (and with it,
// !rcon-style redirect_print capture) instead of going straight to the engine via the
// trampoline. Falls back to the plain engine print in a NOPY build, where the hook
...
// Reads an integer cvar, substituting a default if it is not registered yet.
static inline int cvar_clamped(cvar_t* cvar, int fallback, int lo, int hi) {
    int v = cvar ? cvar->integer : fallback;
    if (v < lo) {
        return lo;
    }
    return (v > hi) ? hi : v;
...
/*
 * Kill a player through G_Damage, the path that raises player_die and with it death/kill.
 * The game module halves self-inflicted damage, hence the doubling; doubled full health lands
 * past the -40 that gibs, so a slay still gibs. MOD_KAMIKAZE gets the 100000 the explosion
 * itself carries.
 */
static inline void SlayPlayer(gentity_t* ent, int mod) {
    int damage = ent->health + (mod == MOD_KAMIKAZE ? 100000 : 0);
...
void __cdecl PyPerfCommand(void);     // "qlx_pyperf"
